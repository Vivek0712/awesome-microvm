# Part 1: form fields

Title (54 chars):
Control and scale AWS Lambda MicroVMs with microvm-ctl

Description (223 chars):
An open-source control and execution plane for Firecracker microVMs on AWS Lambda MicroVMs: images, fleets, tokens, suspend economics, and the quota limits that fresh accounts hit. Every number measured on the live service.

Tags: ["lambda", "serverless", "firecracker", "python", "sandbox"]
Series: Building on AWS Lambda MicroVMs
Cover image: blog/img/cover-00.png

Images to upload, in body order (each replaces the matching ![...](img/...) line):
  1. blog/img/arch-00-plane.png
  2. blog/img/mvm-image-ls.png
  3. blog/img/benchmark.png
  4. blog/img/lifecycle.png
  5. blog/img/mvm-cost.png
  6. blog/img/mvm-quotas.png
